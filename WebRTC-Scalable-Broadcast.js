  ...
  io.on("connection", function (socket) {
    ...
    socket.on("join-broadcast", function (user) {
      ...
      if (firstAvailableBroadcaster) {
        listOfBroadcasts[user.broadcastid].broadcasters[
          firstAvailableBroadcaster.userid
        ].numberOfViewers++;
        socket.emit(
          "join-broadcaster",
          firstAvailableBroadcaster,
          listOfBroadcasts[user.broadcastid].typeOfStreams
        );

        console.log(
          "User <",
          user.userid,
          "> is trying to get stream from user <",
          firstAvailableBroadcaster.userid,
          ">"
        );
      } else {
        ...
        console.log(
          "User <",
          user.userid,
          "> will be next to serve broadcast."
        );
      }
    ...
    socket.on("disconnect", function () {
      ...
      if (
        !listOfBroadcasts[currentUser.broadcastid].broadcasters[
          currentUser.userid
        ]
      )
        return;

      delete listOfBroadcasts[
        currentUser.broadcastid
      ].broadcasters[currentUser.userid];
