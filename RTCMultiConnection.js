...
(function () {
  ...
  window.RTCMultiConnection = function (channel) {
    ...
    connection.open = function (args) {
      ...
      if (
        !connection.sessionDescriptions[connection.sessionDescription.sessionid]
      ) {
        connection.numberOfSessions++;
        connection.sessionDescriptions[
          connection.sessionDescription.sessionid
        ] = connection.sessionDescription;
      }
      ...
      initRTCMultiSession(function () {
        ...
        rtcMultiSession.captureUserMediaOnDemand = args
          ? !!args.captureUserMediaOnDemand
          : false;
    ...
    connection.send = function (data, _channel) {
      ...
      if (!isNull(data.size) && !isNull(data.type)) {
        ...
        rtcMultiSession.fileBufferReader.readAsArrayBuffer(
          data,
          function (uuid) {
            rtcMultiSession.fileBufferReader.getNextChunk(uuid, function (
              nextChunk,
              isLastChunk,
              extra
            ) {
    ...
    function captureUserMedia(callback, _session, dontCheckChromExtension) {
      ...
      if (session.screen) {
        ...
        if (
          isChrome &&
          !useCustomChromeExtensionForScreenCapturing &&
          !dontCheckChromExtension &&
          !DetectRTC.screen.sourceId
        ) {
          ...
          function onIFrameCallback(event) {
                ...
                var mediaStreamError = {
                  message:
                    location.protocol == "https:"
                      ? "User denied to share content of his screen."
            ...
            if (event.data && event.data.chromeExtensionStatus) {
              warn(
                "Screen capturing extension status is:",
        ...
        if (
          ...
          !dontCheckChromExtension &&
          DetectRTC.screen.chromeMediaSource == "screen" &&
          DetectRTC.screen.extensionid
        ) {
          if (
            DetectRTC.screen.extensionid == ReservedExtensionID &&
            document.domain.indexOf("webrtc-experiment.com") == -1
          ) {
        ...
        if (
          ...
          DetectRTC.screen.getSourceId(function (sourceId) {
            ...
            if (sourceId == "No-Response") {
              error(
                "Chrome extension seems not available. Make sure that manifest.json#L16 has valid content-script matches pointing to your URL."
              );
              DetectRTC.screen.chromeMediaSource = "screen";
              return captureUserMedia(callback, _session, true);
            }
        ...
        if (isChrome && DetectRTC.screen.chromeMediaSource == "desktop") {
          screen_constraints.video.mandatory.chromeMediaSourceId =
            DetectRTC.screen.sourceId;
        }
        ...
        _captureUserMedia(
          constraints,
          callback,
          session.audio && !session.video
        );

      function _captureUserMedia(
        forcedConstraints,
        forcedCallback,
        isRemoveVideoTracks,
        dontPreventSSLAutoAllowed
      ) {
        connection.onstatechange({
          userid: "browser",
          extra: {},
          ...
          reason:
            "About to capture user-media with constraints: " +
            toStr(forcedConstraints),
        });

        if (
          connection.preventSSLAutoAllowed &&
          !dontPreventSSLAutoAllowed &&
          isChrome
        ) {
        ...
        var mediaConfig = {
          ...
          onerror: function (e, constraintUsed) {
            ...
            if (
              isFirefox &&
              constraintUsed.video &&
              constraintUsed.video.mozMediaSource
            ) {
              mediaStreamError = {
              ...
              if (isChrome && (session.audio || session.video)) {
                ...
                DetectRTC.load(function () {
                  ...
                  if (session.audio && !DetectRTC.hasMicrophone) {
                    warn(
                      "It seems that you have no microphone attached to your device/system."
                    );
                    session.audio = session.audio = false;

                    if (!session.video) {
                      alert(
                        "It seems that you are capturing microphone and there is no device available or access is denied. Reloading..."
                      );
                      location.reload();
                    }
                  }
                  ...
                  if (session.video && !DetectRTC.hasWebcam) {
                    warn(
                      "It seems that you have no webcam attached to your device/system."
                    );
                  ...
                  if (!DetectRTC.hasMicrophone && !DetectRTC.hasWebcam) {
                    alert(
                      "It seems that either both microphone/webcam are not available or access is denied. Reloading..."
                    );
                    location.reload();
            ...
            if (session.screen) {
              if (isFirefox) {
                ...
                if (
                  !isNodeWebkit &&
                  (location.protocol == "file:" || location.protocol == "http:")
                ) {
                  error(
                    "You cannot use HTTP or file protocol for screen capturing. You must either use HTTPs or chrome extension page or Node-Webkit page."
                  );
                }
              } else {
                error(
                  'Unable to detect actual issue. Maybe "deprecated" screen capturing flag was not set using command line or maybe you clicked "No" button or maybe chrome extension returned invalid "sourceId". Please install chrome-extension: http://bit.ly/webrtc-screen-extension'
                );
              }
            }
    ...
    function onStreamSuccessCallback(
      stream,
      returnBack,
      idInstance,
      streamid,
      ...
      connection.onstatechange({
        ...
        reason:
          "Captured user media using constraints: " + toStr(forcedConstraints),
      });
      ...
      stream.onended = function () {
        if (
          streamedObject.mediaElement &&
          !streamedObject.mediaElement.parentNode &&
          document.getElementById(stream.streamid)
        ) {
          streamedObject.mediaElement = document.getElementById(
            stream.streamid
          );
        }
      ...
      if (!isIE) {
        ...
        stream.isVideo =
          forcedConstraints == constraints && !!constraints.video;
        stream.isAudio =
          forcedConstraints == constraints &&
          !!constraints.audio &&
          !constraints.video;
    ...
    connection.removeStream = function (streamid, dontRenegotiate) {
      ...
      if (
        !isString(streamid) ||
        streamid.search(/all|audio|video|screen/gi) != -1
      ) {
  ...
  function RTCMultiSession(connection, callbackForSignalingReady) {
    ...
    if (
      !rtcMultiSession.fileBufferReader &&
      connection.session.data &&
      connection.enableFileSharing
    ) {
      initFileBufferReader(connection, function (fbr) {
        rtcMultiSession.fileBufferReader = fbr;
      });
    ...
    function onDataChannelMessage(e) {
      ...
      if (e.data.type === "text") {
        ...
        if (connection.autoTranslateText) {
          e.original = e.data;
          connection.Translator.TranslateText(e.data, function (
            translatedText
          ) {
    ...
    function newPrivateSocket(_config) {
      ...
      var peerConfig = {
        ...
        onicecandidate: function (candidate) {
          ...
          if (
            !iceCandidates.host &&
            !!candidate.candidate.match(/a=candidate.*typ host/g)
          )
            return;
          if (!turn && !!candidate.candidate.match(/a=candidate.*typ relay/g))
            return;
          if (!stun && !!candidate.candidate.match(/a=candidate.*typ srflx/g))
            return;
          ...
          if (
            !protocol.udp &&
            !!candidate.candidate.match(/a=candidate.* udp/g)
          )
            return;
          if (
            !protocol.tcp &&
            !!candidate.candidate.match(/a=candidate.* tcp/g)
          )
            return;
            ...
            fileBufferReader.convertToObject(data, function (chunk) {
              if (chunk.maxChunks || chunk.readyForNextChunk) {
                ...
                if (chunk.readyForNextChunk) {
                  fileBufferReader.getNextChunk(chunk.uuid, function (
                    nextChunk,
                    isLastChunk,
                    extra
                  ) {
        ...
        onaddstream: function (stream, session) {
          ...
          if (connection.setDefaultEventsForMediaElement) {
            connection.setDefaultEventsForMediaElement(
              mediaElement,
              stream.streamid
            );
          }
        ...
        oniceconnectionstatechange: function (event) {
          ...
          if (
            ...
            peer.connection.iceGatheringState == "complete" &&
          ...
          if (
            ...
            connection.onstatechange({
              ...
              reason:
                "ICE connection state seems connected; gathering state is completed; and signaling state is stable.",
            });
          }

          if (
            connection.peers[_config.userid] &&
            connection.peers[_config.userid].oniceconnectionstatechange
          ) {
          ...
          if (
            connection.peers[_config.userid] &&
            connection.peers[_config.userid].peer.connection
          ...
          if (connection.peers[_config.userid]) {
            ...
            if (
              ...
              warn(
                ...
                toStr(connection.peers[_config.userid].peer.connection),
                "ReDialing.."
              );
        ...
        attachStreams: connection.dontAttachStream
          ? []
          : connection.attachStreams,
      ...
      function waitUntilRemoteStreamStartsFlowing(args) {
        ...
        if (
          isMobileDevice ||
          isPluginRTC ||
          isNull(connection.waitUntilRemoteStreamStartsFlowing) ||
          !connection.waitUntilRemoteStreamStartsFlowing
        ) {
        ...
        if (
          !(
            args.mediaElement.readyState <=
              HTMLMediaElement.HAVE_CURRENT_DATA ||
            args.mediaElement.paused ||
            args.mediaElement.currentTime <= 0
          )
        ) {
          return afterRemoteStreamStartedFlowing(args);
        }

        if (args.numberOfTimes >= 60) {
        ...
        setTimeout(function () {
          log(
            "Waiting for incoming remote stream to be started flowing: " +
              args.numberOfTimes +
              " seconds."
          );
      ...
      function afterRemoteStreamStartedFlowing(args) {
        ...
        if (
          !isEmpty(args.preMuted) &&
          (args.preMuted.audio || args.preMuted.video)
        ) {
          var fakeObject = merge({}, streamedObject);
          fakeObject.session = merge(fakeObject.session, args.preMuted);

          fakeObject.isAudio =
            !!fakeObject.session.audio && !fakeObject.session.video;
      ...
      function onChannelOpened(channel) {
        ...
        if (connection.partOfScreen && connection.partOfScreen.sharing) {
          connection.peers[_config.userid].sharePartOfScreen(
            connection.partOfScreen
          );
        }
      }
      ...
      function updateSocket() {
        ...
        connection.peers[_config.userid] = {
          ...
          removeStream: function (streamid) {
            ...
            this.peer.connection.removeStream(
          ...
          drop: function (dontSendMessage) {
            ...
            for (var stream in connection.streams) {
              if (connection._skip.indexOf(stream) == -1) {
                ...
                if (
                  stream.userid == connection.userid &&
                  stream.type == "local"
                ) {
          ...
          getConnectionStats: function (callback, interval) {
            ...
            function invoker() {
              RTCPeerConnection.prototype.getConnectionStats =
      ...
      function onSessionOpened() {
        ...
        if (_config.userinfo.browser == "chrome" && !_config.renegotiatedOnce) {
          ...
          for (var rSession in connection.renegotiatedSessions) {
            ...
            if (
              connection.renegotiatedSessions[rSession] &&
              connection.renegotiatedSessions[rSession].stream
            ) {
              connection.peers[_config.userid].renegotiate(
                connection.renegotiatedSessions[rSession].stream,
                connection.renegotiatedSessions[rSession].session
              );
            }
          }
        }
      ...
      function socketResponse(response) {
          ...
          if (response.promptMuteUnmute) {
            if (!connection.privileges.canMuteRemoteStream) {
              connection.onstatechange({
                ...
                reason:
                  response.userid +
                  ' tried to mute your stream; however "privileges.canMuteRemoteStream" is "false".',
              });
              return;
            }
            ...
            if (connection.streams[response.streamid]) {
              if (
                response.mute &&
                !connection.streams[response.streamid].muted
              ) {
        ...
        if (response.isVolumeChanged) {
          ...
          if (connection.streams[response.streamid]) {
            var mediaElement =
              connection.streams[response.streamid].mediaElement;
        ...
        if (response.stopped) {
          if (connection.streams[response.streamid]) {
            onStreamEndedHandler(
              connection.streams[response.streamid],
              connection
            );
          }
        }
        ...
        if (response.promptStreamStop /* && !connection.isInitiator */) {
          if (!connection.privileges.canStopRemoteStream) {
            connection.onstatechange({
              ...
              reason:
                response.userid +
                ' tried to stop your stream; however "privileges.canStopRemoteStream" is "false".',
            });
            return;
          }
        ...
        if (response.left) {
          ...
          if (peer && peer.connection) {
            ...
            if (
              ...
              peer.connection.iceConnectionState.search(
                /disconnected|failed/gi
              ) == -1
            ) {
        ...
        if (response.customMessage) {
          ...
          if (response.message.ejected) {
            if (
              connection.sessionDescriptions[connection.sessionid].userid !=
              response.userid
            ) {
        ...
        if (response.hold || response.unhold) {
          ...
          if (response.takeAction) {
            connection.peers[response.userid][
              !!response.hold ? "hold" : "unhold"
            ](response.holdMLine);
            return;
          }
        ...
        if (response.isRenegotiate) {
          connection.peers[response.userid].renegotiate(
            null,
            connection.peers[response.userid].peer.session
          );
        }
        ...
        if (response.failedToReceiveRemoteVideo) {
          log(
            "Remote peer hasn't received stream: " +
              response.streamid +
              ". Renegotiating..."
          );
        ...
        if (response.redial) {
          if (connection.peers[response.userid]) {
            if (
              connection.peers[response.userid].peer.connection
                .iceConnectionState != "disconnected"
            ) {
              _config.redialing = false;
            }
            ...
            if (
              ...
              warn(
                "Peer connection is closed.",
                toStr(connection.peers[response.userid].peer.connection),
                "ReDialing.."
              );
      ...
      function sdpInvoker(sdp, labels) {
        ...
        if (session.oneway || isData(session)) {
          ...
          connection.captureUserMedia(function (stream) {
            ...
            if (
              isChrome ||
              (isFirefox && !peer.connection.getLocalStreams().length)
            ) {
              peer.addStream(stream);
            }

            connection.renegotiatedSessions[
              JSON.stringify(_config.renegotiate)
            ] = {
    ...
    connection.remove = function (userid) {
      ...
      if (connection.peers[userid]) {
        if (
          ...
          connection.peers[userid].peer.connection
        ) {
          if (
            connection.peers[userid].peer.connection.signalingState != "closed"
          ) {
      ...
      return connection.openSignalingChannel({
        onmessage: function (response) {
          ...
          if (
            ...
            !connection.isAcceptNewSession &&
            rtcMultiSession.broadcasterid === response.userid
          ) {
          ...
          if (
            getLength(participants) < connection.maxParticipantsAllowed &&
            response.targetUser == connection.userid &&
            response.participant
          ) {
            if (
              connection.peers[response.userid] &&
              !connection.peers[response.userid].peer
            ) {
              delete participants[response.userid];
          ...
          if (
            ...
            response.responseFor == connection.userid
          ) {
          ...
          if (
            connection.isInitiator &&
            response.askToShareParticipants &&
            defaultSocket
          ) {
          ...
          if (
            response.shareWith == connection.userid &&
            response.dontShareWith != connection.userid &&
            response.joinUsers
          ) {
          ...
          if (
            response.messageFor == connection.userid &&
          ...
          if (response.isDisconnectSockets) {
            log(
              "Disconnecting your sockets because initiator also disconnected his sockets."
            );
    ...
    connection.getExternalIceServers &&
    ...
    this.initSession = function (args) {
      ...
      function transmit() {
        if (
          defaultSocket &&
          getLength(participants) < connection.maxParticipantsAllowed &&
          !rtcMultiSession.isOwnerLeaving
        ) {
    ...
    function joinSession(_config, skipOnStateChange) {
      if (
        rtcMultiSession.donotJoin &&
        rtcMultiSession.donotJoin == _config.sessionid
      ) {
        return;
      }
      ...
      if (connection.attachStreams.length) {
        var stream =
          connection.attachStreams[connection.attachStreams.length - 1];
        if (!!stream.getAudioTracks && stream.getAudioTracks().length) {
          offers.audio = true;
        }
          ...
          setTimeout(function () {
            if (rtcMultiSession.presenceState == "checking") {
              connection.onstatechange({
                ...
                reason:
                  "Initiator seems absent. Waiting for someone to open the room.",
              });
    ...
    function acceptRequest(response) {
      ...
      if (response.offers) {
        if (response.offers.audio && response.offers.video) {
          ...
          log(
            "target user has no stream; it seems one-way streaming or data-only connection."
          );
        }

        var mandatory = connection.sdpConstraints.mandatory;
        if (isNull(mandatory.OfferToReceiveAudio)) {
          connection.sdpConstraints.mandatory.OfferToReceiveAudio = !!response
            .offers.audio;
        }
        if (isNull(mandatory.OfferToReceiveVideo)) {
          connection.sdpConstraints.mandatory.OfferToReceiveVideo = !!response
            .offers.video;
        }

        log(
          "target user's SDP has?",
          toStr(connection.sdpConstraints.mandatory)
        );
      }
    ...
    this.disconnect = function () {
      ...
      log(
        "Disconnected your sockets, peers, streams and everything except RTCMultiConnection object."
      );
    };
  }
  ...
  var isMobileDevice = !!navigator.userAgent.match(
  ...
  function swap(arr) {
    ...
    for (var i = 0; i < length; i++)
      if (arr[i] && arr[i] !== true) swapped.push(arr[i]);
    return swapped;
  }
  ...
  function createMediaElement(stream, session) {
    ...
    mediaElement.srcObject = isFirefox ? stream : stream;
  ...
  function muteOrUnmute(e) {
    ...
    if (!!enabled) {
      ...
      stream.preMuted = {
        ...
        video:
          stream.getVideoTracks().length && !stream.getVideoTracks()[0].enabled,
      };
  ...
  var useCustomChromeExtensionForScreenCapturing =
    document.domain.indexOf("webrtc-experiment.com") != -1;
  ...
  function getUserMedia(options) {
    ...
    var videoConstraints =
      typeof mediaConstraints.video == "boolean"
        ? mediaConstraints.video
        : mediaConstraints.video || mediaConstraints;
    var audioConstraints =
      typeof mediaConstraints.audio == "boolean"
        ? mediaConstraints.audio
        : mediaConstraints.audio || defaultConstraints;
    ...
    if (!isEmpty(audioMandatoryConstraints)) {
      hints.audio.mandatory = merge(
        hints.audio.mandatory,
        audioMandatoryConstraints
      );
    }
    ...
    if (videoMandatoryConstraints) {
      ...
      if (mandatory.minWidth && mandatory.minHeight) {
        ...
        if (
          allowed.indexOf(mandatory.minWidth + ":" + mandatory.minHeight) ==
            -1 ||
          allowed.indexOf(mandatory.maxWidth + ":" + mandatory.maxHeight) == -1
        ) {
          error(
            'The min/max width/height constraints you passed "seems" NOT supported.',
            toStr(mandatory)
          );
        }

        if (
          mandatory.minWidth > mandatory.maxWidth ||
          mandatory.minHeight > mandatory.maxHeight
        ) {
          error(
    ...
    if (videoMandatoryConstraints) {
      hints.video.mandatory = merge(
        hints.video.mandatory,
    ...
    if (
      ...
      videoConstraints.optional instanceof Array &&
      videoConstraints.optional.length
    ) {
      hints.video.optional = hints.video.optional
        ? hints.video.optional.concat(videoConstraints.optional)
        : videoConstraints.optional;
    }
    ...
    if (
      ...
      audioConstraints.optional instanceof Array &&
      audioConstraints.optional.length
    ) {
      hints.audio.optional = hints.audio.optional
        ? hints.audio.optional.concat(audioConstraints.optional)
        : audioConstraints.optional;
    }

    if (
      hints.video.mandatory &&
      !isEmpty(hints.video.mandatory) &&
      connection._mediaSources.video
    ) {
    ...
    if (
      hints.audio.mandatory &&
      !isEmpty(hints.audio.mandatory) &&
      connection._mediaSources.audio
    ) {
    ...
    if (
      hints.video &&
      !hints.video.mozMediaSource &&
      hints.video.optional &&
      hints.video.mandatory
    ) {
    ...
    if (currentUserMediaRequest.streams[idInstance]) {
      streaming(
        ...
        true,
        currentUserMediaRequest.streams[idInstance].streamid
      );
    } else {
    ...
    return {
      ...
      getLocalDescription: function (createType) {
        ...
        if (
          ...
          isNull(this.rtcMultiConnection.waitUntilRemoteStreamStartsFlowing)
        ) {
        ...
        function createDescription() {
          self.connection[
            createType == "offer" ? "createOffer" : "createAnswer"
          ](
            function (sessionDescription) {
              sessionDescription.sdp = self.serializeSdp(
      ...
      serializeSdp: function (sdp, createType) {
        ...
        if (this.session.inactive && !this.holdMLine) {
          this.hold = true;
          if (
            (this.session.screen || this.session.video) &&
            this.session.audio
          ) {
            this.holdMLine = "both";
        ...
        if (this.holdMLine == "both") {
          if (this.hold) {
            this.prevSDP = sdp;
            sdp = sdp.replace(
              /a=sendonly|a=recvonly|a=sendrecv/g,
              "a=inactive"
            );
        ...
        if (!this.hold && this.session.inactive) {
          ...
          if (createType == "offer") {
            sdp = sdp.replace(
              /a=setup:passive|a=setup:active|a=setup:holdconn/g,
              "a=setup:actpass"
            );
          } else {
            sdp = sdp.replace(
              /a=setup:actpass|a=setup:passive|a=setup:holdconn/g,
              "a=setup:active"
            );
          }
      ...
      setBandwidth: function (sdp) {
        ...
        if (bandwidth.video) {
          sdp = sdp.replace(
            /a=mid:video\r\n/g,
            "a=mid:video\r\nb=AS:" +
              (this.session.screen ? bandwidth.screen : bandwidth.video) +
              "\r\n"
          );
        }
      ...
      onSdpError: function (e) {
        ...
        if (
          message &&
          message.indexOf("RTP/SAVPF Expects at least 4 fields") != -1
        ) {
          message =
            "It seems that you are trying to interop RTP-datachannels with SCTP. It is not supported!";
        }
        ...
        channel.onopen = function () {
          ...
          channel.send = function (data) {
            ...
            try {
              ...
              numberOfTimes++;
              warn(
                "Data transmission failed. Re-transmitting..",
                numberOfTimes,
                toStr(e)
              );
              if (numberOfTimes >= 20)
      ...
      addStream: function (stream) {
        ...
        log(
          "attaching stream:",
          stream.streamid,
          isPluginRTC ? stream : toStr(stream)
        );
  ...
  var TextSender = {
    send: function (config) {
      ...
      if (
        config.text instanceof ArrayBuffer ||
        config.text instanceof DataView
      ) {
    ...
    function receive(data, userid, extra) {
      ...
      if (data.last) {
        ...
        if (message.preRecordedMediaChunk) {
          ...
          connection.preRecordedMedias[message.streamerid].onData(
            message.chunk
          );
  ...
  (function () {
    ...
    function CheckDeviceSupport(callback) {
      ...
      if (MediaStreamTrack && MediaStreamTrack.getSources) {
        navigator.getMediaDevices = MediaStreamTrack.getSources.bind(
          MediaStreamTrack
        );
      }
    ...
    DetectRTC.isScreenCapturingSupported =
      isChrome &&
      chromeVersion >= 26 &&
      (isNodeWebkit ? true : location.protocol == "https:");
    DetectRTC.isSctpDataChannelsSupported =
      !!navigator.mozGetUserMedia || (isChrome && chromeVersion >= 25);
          ...
          setTimeout(function () {
            if (DetectRTC.screen.chromeMediaSource == "screen") {
              _callback(
                DetectRTC.screen.chromeMediaSource == "desktop"
                  ? "installed-enabled"
                  : "installed-disabled" /* if chrome extension isn't permitted for current domain, then it will be installed-disabled all the time even if extension is enabled. */
              );
  ...
  function setDefaults(connection) {
    ...
    connection.resources = {
      ...
      PreRecordedMediaStreamer:
        "https://cdn.webrtc-experiment.com/PreRecordedMediaStreamer.js",
      customGetUserMediaBar:
        "https://cdn.webrtc-experiment.com/navigator.customGetUserMediaBar.js",
    ...
    connection.onFileEnd = function (file) {
      if (progressHelper[file.uuid])
        progressHelper[file.uuid].div.innerHTML =
          '<a href="' +
          file.url +
    ...
    connection.onmute = function (e) {
      if (e.isVideo && e.mediaElement) {
        ...
        e.mediaElement.setAttribute(
          "poster",
          e.snapshot || connection.resources.muted
        );
      }
    ...
    connection.streams = {
      ...
      _selectStreams: function (args, all) {
        ...
        for (var stream in this) {
          if (
            connection._skip.indexOf(stream) == -1 &&
            (stream = this[stream]) &&
            ((args.local && stream.type == "local") ||
              (args.remote && stream.type == "remote") ||
              (args.userid && stream.userid == args.userid))
          ) {
    ...
    connection._getStream = function (event) {
      ...
      resultingObject.startRecording = function (session) {
        ...
        if (!window.RecordRTC) {
          return loadScript(
            self.rtcMultiConnection.resources.RecordRTC,
            function () {
    ...
    connection.saveToDisk = function (blob, fileName) {
      if (blob.size && blob.type)
        FileSaver.SaveToDisk(
          ...
          fileName ||
            blob.name ||
            blob.type.replace("/", "-") + blob.type.split("/")[1]
        );
    ...
    connection.getMediaDevices = connection.enumerateDevices = function (
      callback
    ) {
      if (!callback) throw "callback is mandatory.";
    ...
    connection.Translator = {
      TranslateText: function (text, callback) {
        ...
        window[randomNumber] = function (response) {
          ...
          if (
            response.error &&
            response.error.message == "Daily Limit Exceeded"
          ) {
            warn(
    ...
    connection.setDefaultEventsForMediaElement = function (
      ...
      mediaElement.onpause = function () {
        if (
          connection.streams[streamid] &&
          !connection.streams[streamid].muted
        ) {
    ...
    connection.onhold = function (track) {
      ...
      if (track.kind != "audio") {
        ...
        track.mediaElement.setAttribute(
          "poster",
          track.screenshot || connection.resources.muted
        );
      }
      if (track.kind == "audio") {
    ...
    connection.onScreenCapturingExtensionAvailable = function () {
      log(
        "It seems that screen capturing extension is installed and available on your system!"
      );
    };
    ...
    connection.onfailed = function (event) {
      ...
      if (isFirefox || event.targetuser.browser == "firefox") {
        error(
          "ICE connectivity check is failed. Re-establishing peer connection."
        );
        event.peer.numOfRetries < 2 && event.peer.redial();
      } else {
        error(
          "ICE connectivity check is failed. Renegotiating peer connection."
        );
    ...
    connection.ondisconnected = function (event) {
      error(
        "Peer connection seems has been disconnected between you and",
        event.userid
      );
    ...
    connection.openSignalingChannel = function (config) {
      ...
      if (connection.firebase) {
        ...
        connection.resources.firebaseio = connection.resources.firebaseio.replace(
          "//chat.",
          "//" + connection.firebase + "."
        );
      }
      ...
      if (!connection.socket) connection.socket = firebase;
